...
export default function Profile() {
  ...
  const fetchData = async () => {
    try {
      const linkResponse = await axios.get(`${config.serverRootURL}/${username}/getLinks`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const actorsWithClicked = similarActors.data.results.map(actor => ({ ...actor, clicked: false }));
  ...
  return (
    ...
    <div className="h-full w-full mx-auto max-w-[1800px] flex flex-col items-center space-y-4">
      ...
      <div className="flex flex-wrap justify-center">
        ...
        <button
          key={index}
          className={`px-4 py-2 rounded-md outline-none font-bold text-white m-2 ${actor.clicked ? 'bg-indigo-500' : 'bg-gray-500'}`}
  ...
  <div className="flex items-center space-x-4">
    {links.length === 0 ? (
      ...
      <>
        <p style={{ fontSize: '20px', fontWeight: 'bold', color: '#4A90E2' }}>LINKS</p>
        {links.map((link, index) => (
          <div key={index} className="flex items-center space-x-2 border rounded-md p-2 m-2">
